...
from authentication.serializers.kb_delete_serializer import KBDeleteSerializer
...
class KBDeleteProxyView(APIView):
    ...
    @swagger_auto_schema(
        tags=["kb"],
        operation_id="kb_delete",
        summary="Deleta um KB (proxy n8n)",
        description="Remove um KB identificado por `hash_id`.",
        request_body=KBDeleteSerializer,
        responses={
            200: openapi.Response(
                description="KB deletado",
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if isinstance(request.data, dict) and request.data:
            ser = KBDeleteSerializer(data=request.data)
            ser.is_valid(raise_exception=True)
            payload = ser.validated_data
        elif "hash_id" in request.query_params:
            payload = {"hash_id": request.query_params.get("hash_id")}
            KBDeleteSerializer(data=payload).is_valid(raise_exception=True)
        else:
            return Response(
