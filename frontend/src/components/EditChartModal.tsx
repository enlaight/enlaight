		...
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent className="max-w-4xl max-h-[80vh] bg-background border border-border">
				...
				<div className="flex" style={{ flexDirection: 'column', gap: 5, marginBottom: 10 }}>
					...
					<Input placeholder={t('dashboard.n8nURL')} value={n8n} onChange={(e) => setN8n(e.target.value)} />
				</div>

				<div className="flex" style={{ flexDirection: 'column', gap: 5, marginBottom: 10 }}>
					<h3>{t('dashboard.htmlCode')}</h3>
					<Textarea placeholder={t('dashboard.htmlCode')} value={html} onChange={(e) => setHTML(e.target.value)} />
				</div>
				...
				<div className="flex items-center" style={{ justifyContent: 'flex-end', gap: 15 }}>
					...
					<Button onClick={handleSave}>{t('dashboard.saveChanges')}</Button>
				</div>
			</DialogContent>
		</Dialog >
	);
